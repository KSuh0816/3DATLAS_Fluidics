...
class Protocol:
    ...
    def evens_and_odds(self, chambers ,tube):
        wait_time = self.hybe_time*3
        self.hybe_volume = 1
        steps = []
        if not self.primed:
            steps.append(self.prime({'TBS':'','WBuffer':'', 'Hybe1':'', 'Hybe2':''},'Waste+2'))
            if not self.simulate:
                self.primed = True
        i = 0
        while (i < 4):
            steps.append(self.replace_volume(chambers,'WBuffer',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(chambers,'Hybe1',self.hybe_volume,speed=self.speed,pause=wait_time))
            steps.append(self.replace_volume(chambers,'WBuffer',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(chambers,'WBuffer',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            
            steps.append(self.replace_volume(chambers,'WBuffer',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(chambers,'Hybe2',self.hybe_volume,speed=self.speed,pause=wait_time))
            steps.append(self.replace_volume(chambers,'WBuffer',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(chambers,'WBuffer',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
    ...
    def dendrimer(self, chambers, tube):
        steps = []
        if not self.primed:
            steps.append(self.prime({'Hybe3':'', 'Hybe1':'', 'Hybe2':''},'Waste+2'))
            if not self.simulate:
                self.primed = True

        wells = [("A", 3), ("B", 6), ("C", 10), ("D", 3), ("E", 6), ("F", 8)] # c should be 8
        ...
        while i <= 8:
            ...
            for well in wells:
                if  well[1] <= i:
                    wells_remaining.append(well[0])
            print(wells_remaining)
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(wells_remaining,'Hybe1',1,speed=self.speed,pause=self.hybe_time))
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(wells_remaining,'Hybe2',1,speed=self.speed,pause=self.hybe_time))
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))

            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(wells_remaining,'Hybe3',1,speed=self.speed,pause=self.hybe_time))
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
            i = i + 1
    ...
    def one_dendrimer(self, chambers, tube):
        steps = []
        if not self.primed:
            steps.append(self.prime({'Hybe3':'', 'Hybe1':'', 'Hybe2':'', 'B':''},'Waste+1'))
            if not self.simulate:
                self.primed = True
        wells_remaining = chambers
        print(wells_remaining)
        ...
        steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
        steps.append(self.replace_volume(wells_remaining,'B',1,speed=self.speed,pause=self.hybe_time))
        steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
        steps.append(self.replace_volume(wells_remaining,'TBS',self.rinse_volume,speed=self.speed,pause=self.rinse_time))
